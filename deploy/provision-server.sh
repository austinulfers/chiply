...
#
#  6. UNMATCHED HOSTNAMES MUST NOT REACH AN APP.
#     When no server_name matches, nginx serves the FIRST-LOADED server block.
#     Before the catch-all below existed that was whichever app sorted first in
#     sites-enabled (megahex), so every bare-IP hit and every scanner probing an
#     unknown hostname was proxied into a real app — which answered 200 with its
#     full page. sites-enabled/000-default-reject now claims default_server on
#     :80 and :443 and refuses both. Never give that block a server_name, and
#     never let an app vhost declare default_server.
#
# ---------------------------------------------------------------------------
...
# Size caps from MEASURED peaks, not guesses. systemd tracks the high-water
# mark for you, so after a few weeks of real traffic:
#     systemctl show <app> -p MemoryPeak -p MemoryMax
# Observed peaks over 35 days (2026-09-02) were 31M megahex, 51M planta-notify,
# 56M veritas-boat-logs — i.e. the original 256M–512M caps were ~10x oversized.
# Current caps, roughly 4–8x peak:
#     megahex            MemoryMax=256M  MemoryHigh=192M
#     planta-notify      MemoryMax=192M  MemoryHigh=144M
#     veritas-boat-logs  MemoryMax=256M  MemoryHigh=192M
#     understudy-website MemoryMax=192M  MemoryHigh=144M
#     chiply             MemoryMax=192M  MemoryHigh=144M
# Sum of all MemoryMax should stay comfortably under ~1.5 GB (currently 1088M,
# so roughly two more small apps fit).
#
# Changing a cap does NOT need a restart: edit the unit, `systemctl daemon-reload`
# and systemd re-applies the cgroup limit to the running service. Confirm with
#     cat /sys/fs/cgroup/system.slice/<app>.service/memory.max
#
# ---------------------------------------------------------------------------
...
#     3003  veritas-boat-logs   veritas.offhourslab.com
#     3004  understudy-website  understudy.offhourslab.com
#     3005  chiply              chiply.offhourslab.com
#     3002  (free — parle-in-paris, decommissioned 2026-09-02)
#
#   The subdomain does not always match the app name (planta-notify serves
#   plants., veritas-boat-logs serves veritas.). The sites-available file is
#   named after the app for megahex/planta-notify but after the subdomain for
#   veritas; that split is historical. Follow whatever the app already uses
#   rather than renaming a live vhost.
#
# ---------------------------------------------------------------------------
...
log "Bounding the systemd journal"
# journald is already FIFO — it drops the OLDEST archived file when a limit is
# hit — but ships with no useful ceiling: SystemMaxUse defaults to 10% of the
# filesystem (~3.8G here). 44 days of logs had reached 262M unchecked.
mkdir -p /etc/systemd/journald.conf.d
cat >/etc/systemd/journald.conf.d/00-size-limit.conf <<'JOURNALD'
# Managed by provision-server.sh.
#   SystemMaxUse       total cap; oldest journal file evicted first
#   SystemMaxFileSize  rotate at this size, so eviction granularity is one file
#   SystemKeepFree     also stop if the disk gets tight, whichever hits first
[Journal]
SystemMaxUse=200M
...
log "Installing catch-all vhost (rule 6)"
# Claims default_server so unmatched hostnames never reach an app. See rule 6.
# The 000- prefix keeps it first in the sites-enabled glob; that is cosmetic,
# default_server is what actually decides this.
cat >/etc/nginx/sites-available/000-default-reject <<'NGINX'
# Catch-all: reject anything that does not match a real app vhost.
# Managed by provision-server.sh. Do NOT add a server_name here — naming a host
# would steal that host from its own app.
#
# ACME renewals are unaffected: certbot's HTTP-01 challenge is addressed to a
# real domain, which matches that app's own :80 block, not this one.
...
server {
    ...
    # 444 = close the connection with no response at all. Cheaper than a 404,
    # and gives scanners nothing to fingerprint.
    return 444;
}
...
server {
    ...
    # Refuse the TLS handshake outright (nginx >= 1.19.4) so we never present
    # an app's certificate to an unmatched request.
    ssl_reject_handshake on;
}
NGINX
ln -sfn /etc/nginx/sites-available/000-default-reject \
        /etc/nginx/sites-enabled/000-default-reject
# Ubuntu's stock default site also claims default_server on :80 and would be a
# duplicate-listen fatal error alongside ours. It is not enabled out of the box
# on this host, but remove the symlink if a rebuild put it back.
